...
By computing 8 wind weights for each cell that affect the spread of fire in the 8 directions of the model, the directional influence the wind has on the fire front can be simulated. To calculate the wind's affect \cite{ALEXANDRIDIS2008191}'s method will be adapted for this implementation as using their weights we can measure the winds affect in meters per second.

For every cell in the grid wind weights are calculated for each of the possible 8 dirrections the fire could be approaching .i.e North, East, North East etc.
...
Where $a_1$ and $a_2$ are weights, the weights calculated by \cite{ALEXANDRIDIS2008191} will be used as they were derived from studying the growth of a real forest fire.\\
These calculations can be done in the preprocessing steps before the simulation runs reducing the computational complexity runtime of the simulation.
...
% This would indicate it would only take 8.5 hours to reach the town?
